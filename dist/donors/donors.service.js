...
let DonorsService = class DonorsService {
    ...
    async findAll(opts) {
        ...
        const q = (opts.q || '').trim();
        if (q) {
            qb.andWhere('(LOWER(COALESCE(d.fullName, \'\')) LIKE :q OR LOWER(COALESCE(d.email, \'\')) LIKE :q OR COALESCE(d.phone, \'\') LIKE :q)', { q: `%${q.toLowerCase()}%` });
        }
        ...
        qb.orderBy(sortMap[sortKey] || 'd.id', sortDir);
    ...
    async stats() {
        ...
        const countsRaw = await this.repo
            ...
            .addSelect(`SUM(CASE WHEN LOWER(COALESCE(d.gender, '')) = 'male' AND d.status = 'accepted' THEN 1 ELSE 0 END)`, 'acceptedMale')
            .addSelect(`SUM(CASE WHEN LOWER(COALESCE(d.gender, '')) = 'female' AND d.status = 'accepted' THEN 1 ELSE 0 END)`, 'acceptedFemale')
            .getRawOne();
        ...
        const bloodTypeRows = await this.repo
            ...
            .groupBy('d.bloodType')
            .getRawMany();
        const byBloodType = (bloodTypeRows || []).reduce((acc, r) => {
            if (!r?.bloodType)
                return acc;
        ...
        const acceptedBloodTypeRows = await this.repo
            ...
            .groupBy('d.bloodType')
            .getRawMany();
        const acceptedByBloodType = (acceptedBloodTypeRows || []).reduce((acc, r) => {
            if (!r?.bloodType)
                return acc;
            acc[r.bloodType] = Number(r.count || 0);
            return acc;
